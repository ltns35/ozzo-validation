...
func TestMap(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			"t2.4",
			m1,
			[]*KeyRules{Key("D", Length(0, 5))},
			"",
		},
		{
			"t2.5",
			m1,
			[]*KeyRules{Key("F", Length(0, 5))},
			"",
		},
		{
			"t2.6",
			m1,
			[]*KeyRules{
				Key("H", Each(&validateAbc{})),
				Key("I", Each(&validateAbc{})),
			},
			"",
		},
		{
			...
			m1,
			[]*KeyRules{
				Key("H", Each(&validateXyz{})),
				Key("I", Each(&validateXyz{})),
			},
		...
		},
		{
			"t2.8",
			m1,
			[]*KeyRules{Key("I", Map(Key("foo", &validateAbc{})))},
			"",
		},
		{
			"t2.9",
		...
		// invalid key spec
		{
			...
			m1,
			[]*KeyRules{Key(123)},
			"123: key not the correct type.",
		},
		{
			"t4.2",
			m1,
			[]*KeyRules{Key("X")},
			"X: required key is missing.",
		},
		{
			"t4.3",
			m1,
			[]*KeyRules{Key("X").Optional()},
			"",
		},
		// non-string keys
		{
		...
		},
		{
			"t6.3",
			m2,
			[]*KeyRules{Key("E", Skip.When(true))},
			"",
		},
		{
			"t6.4",
			m2,
			[]*KeyRules{Key("E", Skip.When(false))},
			"E: error 123.",
		},
		// Required, NotNil
		{
			"t7.1",
			m2,
			[]*KeyRules{Key("F", Required)},
			"F: cannot be blank.",
		},
		{
			"t7.2",
			m2,
			[]*KeyRules{Key("F", NotNil)},
			"F: is required.",
		},
		{
			"t7.3",
			m2,
			[]*KeyRules{Key("F", Skip, Required)},
			"",
		},
		{
			"t7.4",
			m2,
			[]*KeyRules{Key("F", Skip, NotNil)},
			"",
		},
		{
			"t7.5",
			m2,
			[]*KeyRules{Key("F", Skip.When(true), Required)},
			"",
		},
		{
			"t7.6",
			m2,
			[]*KeyRules{Key("F", Skip.When(true), NotNil)},
			"",
		},
		{
			"t7.7",
			m2,
			[]*KeyRules{Key("F", Skip.When(false), Required)},
			"F: cannot be blank.",
		},
		{
			"t7.8",
			m2,
			[]*KeyRules{Key("F", Skip.When(false), NotNil)},
			"F: is required.",
		},
		// validatable structs
		{
...
func TestMapWithContext(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			m1,
			[]*KeyRules{
				Key("A", &validateContextXyz{}),
				Key("B", &validateContextAbc{}),
			},
			"A: error xyz; B: error abc.",
		},
		{
		...
		},
		{
			"t1.4",
			m1,
			[]*KeyRules{Key("g", &validateContextAbc{})},
			"g: error abc.",
		},
		// skip rule
		{
			"t2.1",
			m1,
			[]*KeyRules{Key("g", Skip, &validateContextAbc{})},
			"",
		},
		{
			"t2.2",
			m1,
			[]*KeyRules{Key("g", &validateContextAbc{}, Skip)},
			"g: error abc.",
		},
		// internal error
		{
